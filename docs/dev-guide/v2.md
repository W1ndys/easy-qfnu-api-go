...
| 参数名      | 位置   | 必填 | 说明                         | 示例             |
| ----------- | ------ | ---- | ---------------------------- | ---------------- |
| **X-Token** | Header | 是   | 教务系统的完整 Cookie 字符串 | `JSESSIONID=...` |

### 3.2 中间件拦截

后端已启用 `middleware.AuthRequired`。

- 如果请求未携带 `X-Token`，接口将直接返回 `401` 状态码，业务逻辑不会执行。
- **Handler 写法**: 不需要手动判断 Token 是否为空，直接从 Context 获取：
...
- **文件命名**: `logs/qfnu-api-{时间戳}.log` (每次启动独立文件)。
...
| `CodeAuthExpired`  | 401  | Token 过期/未登录 |
